...
31-30  29-25   24-19     18-14    	13-5             4-0
+----+-------+--------+------------------------------------------+
| 10 | 00000 | 110100 |   rs1   |     001001101      |   rs2	 |
+----+-------+--------+------------------------------------------+
...
#define FCMPS_CODE_MASK 0x81a80a20
#define PRIVILEDGE  0
...
static int execute(void *state)
{
	...
	va_rs1.data = fpu_get_single(rs1);
	...
	print_fpinst_RS_RS("fcmps", rs1, rs2);
	PCREG = NPCREG;
	NPCREG += 4;
...
static int disassemble(uint32 instr, void *state)
{
	...
	if( (instr & FCMPS_CODE_MASK) && (op == OP) && (opf == OPF) )
	{
