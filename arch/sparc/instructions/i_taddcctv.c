 ...
 * =====================================================================================
 *
 *       Filename:  i_taddcctv.c
 *
 *    Description:  Implementation of the TADDCCTV instruction
 *
...
/*  Format (3)
    ...
    31-30  29-25   24-19     18-14    13        12-5          4-0
    +----+-------+--------+------------------------------------------+
    | 10 | 00000 | 100010 |   rs1   | 1/0 |      simm12/rs2          |
    +----+-------+--------+------------------------------------------+
...
#define TADDCCTV_CYCLES    1
#define TADDCCTV_CODE_MASK 0x81100000
#define PRIVILEDGE  0
...
sparc_instruction_t i_taddcctv = {
	execute,
	disassemble,
	TADDCCTV_CODE_MASK,
	PRIVILEDGE,
...
static int execute(void *state)
{
	...
	if( imm < 0 )
	{
		...
		print_inst_RS_RS_RD("taddcctv", rs1, rs2, rd);
	}
	else
	{
		y = sign_ext13(imm);
		z = REG(rd) = REG(rs1) + sign_ext13(imm);
		print_inst_RS_IM13_RD("taddcctv", rs1, sign_ext13(imm), rd);
	}
...
static int disassemble(uint32 instr, void *state)
{
	...
	if( (instr & TADDCCTV_CODE_MASK) && (op == OP) && (op3 == OP3) )
	{
