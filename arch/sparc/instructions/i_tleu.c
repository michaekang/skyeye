 ...
 *       Filename:  i_tleu.c
 *
 *    Description:  Implementation of the TLEU instruction
 *
 *       Compiler:  gcc
 *
...
+----+----+--------+------------------------------------------+
| op | na |  0100  | 111010  |   rs1   |i=1| reserved | imm7  |
+----+----+--------+------------------------------------------+
...
#define TLEU_CYCLES    1
#define TLEU_CODE_MASK 0x89d00000
#define PRIVILEDGE  0
...
static int disassemble(uint32 instr, void *state)
{
	...
	if( (instr & TLEU_CODE_MASK) && (op == OP) && (fixed == FIXED) )
	{
