 ...
 *       Filename:  i_tne.c
 *
 *    Description:  Implementation of the TNE instruction
 *
 *       Compiler:  gcc
 *
...
+----+----+--------+------------------------------------------+
| op | na |  1001 | 111010  |   rs1   |i=1| reserved | imm7  |
+----+----+--------+------------------------------------------+
...
#define TNE_CYCLES    1
#define TNE_CODE_MASK 0x93d00000
#define PRIVILEDGE  0
...
static int disassemble(uint32 instr, void *state)
{
	...
	if( (instr & TNE_CODE_MASK) && (op == OP) && (fixed == FIXED) )
	{
