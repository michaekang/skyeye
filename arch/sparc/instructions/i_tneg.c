 ...
 *       Filename:  i_tneg.c
 *
 *    Description:  Implementation of the TNEG instruction
 *
 *       Compiler:  gcc
 *
...
+----+----+--------+------------------------------------------+
| op | na |  0110  | 111010  |   rs1   |i=1| reserved | imm7  |
+----+----+--------+------------------------------------------+
...
#define TNEG_CYCLES    1
#define TNEG_CODE_MASK 0x8dd00000
#define PRIVILEDGE  0
...
static int disassemble(uint32 instr, void *state)
{
	...
	if( (instr & TNEG_CODE_MASK) && (op == OP) && (fixed == FIXED) )
	{
