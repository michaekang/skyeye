...
:sc_module   (module_name)
{
	...
	target_socket.register_b_transport(this, &Lt_bus_target::custom_b_transport);
}

Lt_bus_target::~Lt_bus_target()
{
}
#if 1 
void Lt_bus_target::custom_b_transport( tlm:: tlm_generic_payload & payload, sc_core :: sc_time & delay_time)
{
...
int Lt_bus_target::bus_read(short size, generic_address_t addr, uint32_t * value)
{
	int res = default_bus_read( size,  addr,   value);
	return res;
}

int Lt_bus_target::bus_write(short size, generic_address_t addr, uint32_t value)
{
